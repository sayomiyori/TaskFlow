...
def test_register_success(api_client):
    # Arrange
    payload = {"email": "newuser@example.com", "password": "StrongPassword123", "name": "New User"}
...
def test_register_duplicate_email(api_client):
    # Arrange
    payload = {"email": "dup@example.com", "password": "StrongPassword123", "name": "Dup User"}
...
def test_login_success(api_client):
    # Arrange
    register_payload = {"email": "loginok@example.com", "password": "StrongPassword123", "name": "Login User"}
...
def test_login_wrong_password(api_client):
    # Arrange
    register_payload = {"email": "loginbad@example.com", "password": "StrongPassword123", "name": "Login Bad"}
    api_client.post("/api/v1/auth/register/", register_payload, format="json")
...
def test_refresh_token(api_client):
    # Arrange
    register_payload = {"email": "refresh@example.com", "password": "StrongPassword123", "name": "Refresh User"}
    api_client.post("/api/v1/auth/register/", register_payload, format="json")
    ...
    # Act
    response = api_client.post("/api/v1/auth/refresh/", {"refresh": login_response.data["refresh"]}, format="json")
...
def test_access_expired(api_client, member_user):
    ...
    expired_access.set_exp(from_time=timezone.now() - timedelta(minutes=10), lifetime=timedelta(seconds=1))
